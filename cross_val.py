...
def get_feature(arg):
    if arg.data_name == 'ACP240':
        tmpdir = "Anti-cancer-data/"
        postrain = glob.glob(tmpdir + '/ACP240/ACP240_positive*')
        negtrain = glob.glob(tmpdir + '/ACP240/ACP240_negative*')
        data_len = 240
        min_len = 11
        # seed = 10
        ...
        tmpdir = "Anti-cancer-data/"
        postrain = glob.glob(tmpdir + '/ACP740/ACP740_positive*')
        negtrain = glob.glob(tmpdir + '/ACP740/ACP740_negative*')
        data_len = 740
        min_len = 11
        # seed = 13
    else:
    ...
    method_peptide = [
        "kmer", "DR.csv",  # 1
        "-PSSM-DT.csv", "-CC-PSSM.csv", "-AC-PSSM.csv", "feature-DT.csv", "-Top-n-gram.csv", "-PDT-Profile.csv", "DP.csv",
        "ACC-PSSM.csv",  # 4
    ...
    return datadics, data_len


def cross_val(arg):
    cvdatadics = {}
    datadics, data_len = get_feature(arg)
    setup_seed(arg.seed)
    KF = KFold(n_splits=5, shuffle=True, random_state=arg.seed)
    k = 1
    all_evaluation = []
